...
import { authenticate, hash, requireAuth, AUTH_COOKIE, networkDelay, encode, decode } from '../utils';
...
const isValidToken = (token: string, type: 'reset' | 'verify'): { email: string } | null => {
  try {
    const data = decode(token) as { email: string; type: string; exp: number };
