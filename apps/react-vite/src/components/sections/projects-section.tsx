            ...
            <Reveal key={index} direction={project.direction} staggerIndex={index}>
              <GlassCard className='group overflow-hidden rounded-2xl' glow>
                <div className='relative h-56 overflow-hidden'>
                  <img src={project.image} alt={project.title} className='project-card-img size-full object-cover' />
                  ...
                  <div
                    className={`absolute left-4 top-4 flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider text-white shadow-lg ${
                      project.tagType === 'gradient' ? 'bg-gradient-to-r from-cyan-500 to-ember-500' : 'bg-red-500'
                    }`}
                  >
                ...
                <div className='p-6'>
                  <div className='mb-3 flex items-center gap-2'>
                    <span className={`relative flex size-2 ${project.statusColor === 'amber' ? 'animate-ping' : ''}`}>
                  ...
                  <div className='mb-5 flex flex-wrap gap-2'>
                    {project.tags.map((tag, tagIndex) => (
                      <Badge key={tagIndex} variant={tagIndex < 2 ? 'cyan' : ('ember' as 'cyan' | 'ember')}>
                        {tag}
                      </Badge>
                    ))}
                  </div>
