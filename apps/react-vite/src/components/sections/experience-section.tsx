...
function highlightText(text: string): ReactNode[] {
  ...
  return parts.map((part, i) => {
    if (part.startsWith('**') && part.endsWith('**')) {
      return (
        <strong key={i} className='font-semibold text-ink-700'>
          {part.slice(2, -2)}
        </strong>
      );
    }
    ...
    <section id='experience' className='relative py-24 md:py-32'>
      ...
      <div className='relative mx-auto max-w-4xl px-6'>
        ...
        <div className='relative'>
          <svg className='absolute inset-y-0 left-[24px] z-[1] w-[2px] md:left-[24px]' preserveAspectRatio='none'>
            <line
              x1='1'
              y1='0'
              x2='1'
          ...
          <div className='space-y-12'>
            {experiences.map((exp, index) => (
              <Reveal key={index} direction='left' staggerIndex={index}>
                <div className='relative'>
                  <div className={`timeline-node ${exp.isCurrent ? 'active-node' : ''}`} style={{ top: '8px' }}></div>
